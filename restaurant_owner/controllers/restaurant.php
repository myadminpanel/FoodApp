...
class dashboard
{
    ...
    {
        $db = getDB();
        $stmt = $db->prepare("SELECT * FROM fooddelivery_res_owner WHERE id=:id");
...
*/
    ...
    public function addnewmenu($urlid,$mname)
    {
        $db = getDB();
        $time=time();
    ...
    public function getallmenu($search,$check,$start,$per_page,$uid)
    {
        $db = getDB();
        if($check == "total")
        {
    ...
    public function addnewboy($urlid,$name,$phone,$email,$password,$vehicle_no,$vehicle_type)
    {
        $db = getDB();
        $time=time();
        $stmt = $db->prepare("INSERT INTO `fooddelivery_delivery_boy`(`res_id`,`name`,`phone`,`email`,`password`,`vehicle_no`,`vehicle_type`,`created_at`) VALUES (:res_id,:name,:phone,:email,:password,:vehicle_no,:vehicle_type,:time)");
        $stmt->bindParam("res_id", $urlid,PDO::PARAM_STR);
    ...
    public function editboydetail($boy_id,$name,$phone,$email,$password,$vehicle_no,$vehicle_type){
    ...
    public function orderassign($order_id,$boy_id){

        $db = getDB();
        //echo "UPDATE `fooddelivery_bookorder` SET `is_assigned`='".$boy_id."' WHERE `id`='".$order_id."'"; exit();
        $date = date('d-m-Y H:i');
        $stmt = $db->prepare("UPDATE `fooddelivery_bookorder` SET status='5',assign_date_time='".$date."',assign_status='active',`is_assigned`='".$boy_id."' WHERE `id`='".$order_id."'");
        $stmt->bindParam("is_assigned", $boy_id, PDO::PARAM_STR);
    ...
    public function updaterespassword($id,$newpass){
    ...
    public function addnewresowneraccess($name,$password,$time,$mobile,$email)
    {
        $db = getDB();
        $time=time();
        $stmt = $db->prepare("INSERT INTO `fooddelivery_res_owner`(`username`,`password`,`timestamp`,`phone`,`email`) VALUES (:name,:password,:time,:mobile,:email)");
        $stmt->bindParam("name", $name,PDO::PARAM_STR);
        $stmt->bindParam("password", $password,PDO::PARAM_STR);
        $stmt->bindParam("time", $time,PDO::PARAM_STR);
