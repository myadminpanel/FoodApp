...
if(isset($_POST['addnewmenu']))
{
    ...
    if(isset($_POST['cname']) && $_POST['cname']!="")
    {
        extract($_REQUEST);
        $created_at=time();
        $user->addnewsubcategory($id,$cname,$created_at);
        if($user)
        {
            ?>
            <script>
                window.location='subcategory.php?<?php echo $_SERVER['QUERY_STRING']; ?>';
            </script>
            <?php
        }
        else
...
{
    extract($_REQUEST);
    $edtmenu = $user->editsubcategorydetail($subcat_id,$cname);
    if($edtmenu)
    {
        ?>
        <script>
...
<head>
    <meta charset="utf-8" />
    <title>Food Delivery System | Sub Category</title>
...
<div id="page-container" class="page-container fade page-without-sidebar page-header-fixed page-with-top-menu">
    ...
    <div id="content" class="content">
        <ol class="breadcrumb pull-right">
            <li><a href="dashboard.php">Home</a> /<a href="category.php"> Category</a> / Restaurant Sub Category</li>
            <!--<li class="active">Dashboard</li>-->
        </ol>
        <h1 class="page-header">Search Sub Category </small></h1>
        <div class="row">
            <div class="col-md-12">
                <div class="result-container">
                    <form method="get">
                        <div class="input-group m-b-20">
                            ...
                            <div class="input-group-btn">
                                ...
                                <a href="subcategory.php?alexe0sdsc=<?php echo $_GET['alexe0sdsc'];?>" type="submit" class="btn btn-inverse"> Reset Filter</a>
                            </div>
                        </div>
                    </form>
                    <div class="btn-group m-l-10 m-b-20">
                        <a href="#addmenu"  data-toggle="modal" class="btn btn-white btn-white-without-border">Add New Sub Category</a>
                    </div>
                    <?php

                    $per_page = 10;
                    $url="subcategory.php?alexe0sdsc=".$_GET['alexe0sdsc']."&";
                    if(isset($_GET['page']))
                    {
                        $pageset = $_GET['page'];
                        if ($pageset == 1)
                    ...
                    {
                        $search=$_GET['search'];
                        $qutotal= $user->getsubcategory($search,"searchtotal","none","none",$id);
                        $query = $user->getsubcategory($search,"search",$start,$per_page,$id);
                    }
                    else
                    {
                        $query = $user->getsubcategory("none","none",$start,$per_page,$id);
                        $qutotal= $user->getsubcategory("none","total","none","none",$id);
                    } ?>
                        ...
                        foreach ($query as $val)
                        {
                            ...
                            <ul class="list-group list-group-lg no-radius list-email">
                                <li class="list-group-item inverse">
                                    <div class="email-user" style="background-color:#1F2429;color: white;border-radius: 0px;">
                                        <?php
                                        $smname=$val['name'];
                                    ...
                                    <div class="email-info">
                                        ...
                                        <p class="email-desc">
                                            <?php echo date("d-M-Y",$val['created_at']);?>
                                        </p>
                                    </div>
                                </li>
                            </ul>
                        ...
                        <div class="panel-body panel-form">
                            <form class="form-horizontal form-bordered" data-parsley-validate="true" method="post" action="" name="demo-form">
                                ...
                                <div class="form-group">
                                    <label class="control-label col-md-4 col-sm-4" for="fullname">Sub Category Name <span style="color: red;">*</span> :</label>
                                    <div class="col-md-6 col-sm-6">
                                        <input class="form-control" type="text"  name="cname" placeholder="Menu Name" data-parsley-required="true" />
                                    </div>
                                </div>

                                <div class="modal-footer">
                                    <a href="javascript:;" class="btn btn-sm btn-white" data-dismiss="modal">Close</a>
                                    <button type="submit" class="btn btn-primary" name="addnewmenu" id="addmneubutton" >Add Sub Category</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-inverse" data-sortable-id="form-validation-1">
                        <div class="panel-body panel-form">
                            <form class="form-horizontal form-bordered" data-parsley-validate="true" method="post" action="" name="demo-form">
                                ...
                                <div class="modal-footer">
                                    <a href="javascript:;" class="btn btn-sm btn-white" data-dismiss="modal">Close</a>
                                    <button type="submit" class="btn btn-primary" name="editnewmenu" id="editmenubutton" >Edit Subcategory</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script>
    ...
    function deletemenu(id)
    {
        ...
        if(x) {
            $.ajax({
                type: "POST",
                url: "ajax/deletesubcategory.php",
                data: {querystring: id},
                cache: false,
                success: function (data)
                {
                    if (data)
                    {
                        window.location='subcategory.php?alexe0sdsc=<?php echo $_GET['alexe0sdsc']; ?>';
                    }
                    else
                    {
