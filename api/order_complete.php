...
if (isset($_REQUEST["order_id"])) {
    ...
    $date = date('d-m-Y H:i');
        ...
        $sql = mysqli_query($conn,"UPDATE `fooddelivery_bookorder` SET status='4',delivered_date_time='".$date."',delivered_status='active' WHERE `id`='".$order_id."'");

        if (isset($sql)) {
                $arrRecord['success'] = "1";
                $arrRecord['order'] = $complete;
            } else {
                $arrRecord['success'] = "0";
